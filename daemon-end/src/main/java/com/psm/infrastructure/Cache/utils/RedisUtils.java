...
import com.psm.utils.MD5.MD5Utils;
import com.psm.utils.Map.MapUtils;
import com.psm.utils.Redis.RedisAsyncUtils;
    ...
    public static void save(final StringRedisTemplate template, final String key, final Object value, final long timeout, final TimeUnit timeUnit) {
        if (value instanceof String) {
            if (timeout <= 0) {
                ...
                log.info("object save successful,key:{},value:{},no expire time", key, value);
            } else {
                template.opsForValue().set(key, (String) value, timeout, timeUnit);
                log.info("object save successful,key:{},value:{}", key, value);
            }
        } else {
            ...
            if (timeout <= 0) {
                ...
                log.info("object save successful,key:{},value:{},no expire time", key, writeValue);
            } else {
                stringOptional.ifPresent(e -> template.opsForValue().set(key, e, timeout, timeUnit));
                log.info("object save successful,key:{},value:{}", key, writeValue);
            }
        }
    }
