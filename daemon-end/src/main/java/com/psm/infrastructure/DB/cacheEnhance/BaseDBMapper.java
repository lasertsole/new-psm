...
public interface BaseDBMapper<T> extends MPJBaseMapper<T> {
    /**
     * 获取泛型实体类
     * <p>
     * 此方法为帮助方法 非dao方法
    ...
    default Class<T> getEntityClass() {
        return (Class<T>) ReflectionKit.getSuperClassGenericType(this.getClass(), BaseDBMapper.class, 1);
    }
    ...
    default <D> boolean insert(T entity, SFunction<T, ? extends Serializable> fkColumn, BaseDBMapper<D> mapper, SFunction<D, ?> statColumn) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    default <D> boolean insert(T entity, SFunction<T, ? extends Serializable> fkColumn, Class<BaseDBMapper<D>> mapperClazz, SFunction<D, ?> statColumn) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    default <D> boolean deleteById(Serializable id, Serializable fkValue, Class<BaseDBMapper<D>> mapperClazz, SFunction<D, ?> statColumn) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    default <D> boolean deleteById(Serializable id, Serializable fkValue, BaseDBMapper<D> mapper, SFunction<D, ?> statColumn) {
        Class<T> clazz = getEntityClass();
        synchronized (clazz) {
